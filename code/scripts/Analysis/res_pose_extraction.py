...
THRESHOLD=0.3 # 0.5 by default
DEBUG=True # Debug block the output
...
def plot_pose_skeleton(frame_path, pose, idx):
    image=plt.imread(frame_path)	
    fig, ax = plt.subplots()
    ax.imshow(image)
    # ax.set_facecolor('xkcd:black')
    for p in pose:
        color_rgb='#ff'+str('{:#04x}'.format(int(255-255*p[2]))[-2:])[-2:]+'00'
        # ax.scatter(p[0],p[1],alpha=0.5,c=color_rgb)
        if p[2]>0.5:
            ax.scatter(p[0],p[1],alpha=0.5,c='red')
        else:
            ax.scatter(p[0],p[1],s=80, facecolors='none', edgecolors='blue', linestyle='--')
    # ax.set_xlim(0,320)
    # ax.set_ylim(0,240)
    plt.savefig('scripts/Analysis/files/skeleton_show/{}.png'.format(idx))
    pass


def pose_inference(args, frame_paths, det_results, threshold=THRESHOLD):

    shutil.rmtree('scripts/Analysis/files/skeleton_show')
    os.makedirs('scripts/Analysis/files/skeleton_show')
    ...
    for i, (f, d) in enumerate(zip(frame_paths, det_results)):
        # Align input format plot_pose_skeleton(frame_paths[0],pose[0]['keypoints'])
        d = [dict(bbox=x) for x in list(d) if x[-1] > THRESHOLD]
        pose = inference_top_down_pose_model(model, f, d, format='xyxy')[0]
        if len(pose)>0:
            plot_pose_skeleton(frame_paths[i],pose[0]['keypoints'],i)
        for j, item in enumerate(pose):
            for poseid in range(pose[0]['keypoints'].shape[0]):
                if pose[0]['keypoints'][poseid][-1]<=0.5:
...
def train_skeleton():
    ...
    args.output = 'data/ntu_dark_skeletions/train/ntu_dark_skeleton_train_{}_{}.pkl'.format(ENHANCE,THRESHOLD)
    args.skip_postproc = global_args.skip_postproc

    anno_list=[]
    for action in os.listdir('data/ntu_dark/rawframes_train')[7:]:
...
def test_skeleton():
    ...
    args.device = global_args.device
    args.video = global_args.video
    args.output = 'data/ntu_dark_skeletions/test/ntu_dark_skeleton_test_{}_{}.pkl'.format(ENHANCE,THRESHOLD)
    args.skip_postproc = global_args.skip_postproc
